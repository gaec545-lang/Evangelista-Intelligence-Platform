...
export default function ReportCard({ report, onView, onDownload }: ReportCardProps) {
  ...
  return (
    <div className={`bg-white rounded-xl p-5 shadow-sm border-l-4 transition-shadow hover:shadow-md ${
      isInternal ? 'border-yellow-500 border-t border-r border-b border-y-eva-border border-r-eva-border bg-yellow-50/10' : 
                   'border-green-500 border-t border-r border-b border-y-eva-border border-r-eva-border'
    }`}>
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-2">
          {isInternal ? (
            <span className="flex items-center gap-1 text-[10px] uppercase tracking-wider font-bold bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">
              <Lock size={10} /> Interno
            </span>
          ) : (
            <span className="flex items-center gap-1 text-[10px] uppercase tracking-wider font-bold bg-green-100 text-green-800 px-2 py-0.5 rounded">
      ...
      <h4 className="font-brand text-lg text-eva-black mb-1 leading-tight">{report.title}</h4>
