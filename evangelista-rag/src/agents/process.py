...
class ProcessAgent(BaseAgent):
    """Agente especializado en optimización de procesos y metodologías."""

    def __init__(self):
        super().__init__(
            name="process",
            prompt_document_id="EVK-AG-PRO-001"
        )
...
Responde siguiendo estrictamente tu formato de output definido. Usa las fuentes del knowledge base cuando sean relevantes y cítalas."""
    ...
    def _estimate_confidence(self, response: str) -> float:
        ...
        uncertainty_phrases = ["no tengo información", "necesito más datos", "no es posible analizar", "información insuficiente"]
        for phrase in uncertainty_phrases:
            if phrase.lower() in response.lower():
                confidence -= 0.15
        if "proceso" in response.lower() or "etapa" in response.lower():
            confidence += 0.05
    ...
    def _should_escalate(self, response: str) -> bool:
        ...
        other_domain_keywords = ["setup fee", "success fee", "factor gamma", "pricing", "modelo dimensional", "ETL", "SQL"]
    ...
    def _extract_escalation_reason(self, response: str) -> str:
        ...
        return "Tarea fuera del dominio de procesos — requiere otro especialista"
