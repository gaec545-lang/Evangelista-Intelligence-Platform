...
with open("Backend/src/retrieval/hybrid_retriever.py", "r") as f:
    content = f.read()
...
content = re.sub(
    r'(async def _qdrant_search\(\s*self,\s*query: str,\s*agent_name: str,)',
    r'\1\n        client_id: str,',
    content
)

content = re.sub(
    r'(qdrant_task = asyncio.create_task\(self\._qdrant_search\(query, agent_name, top_k=20\)\))',
    r'qdrant_task = asyncio.create_task(self._qdrant_search(query, agent_name, client_id, top_k=20))',
    content
)
...
with open("Backend/src/retrieval/hybrid_retriever.py", "w") as f:
    f.write(content)
